...
class KonfirmasiHotelController extends Controller
{
    public function index() {
        ...
        if ( Auth::user()->role == 2)
        {
            $data_pesan_hotel_d = (new TPesananHotelD)->get_by_payment( 2);   
        }
        ...
        {
            $in = false;
            foreach($data_surat_tugas_h as $surat_tugas_h){
                if($surat_tugas_h->id == $data_d->id_surat_tugas_h)
                {
                    $in = true;
                    $surat_tugas_h['data_pesan_hotel'][count(($surat_tugas_h['data_pesan_hotel'])+1)] = $data_d;
                }
            }
            if( !$in)
            {
